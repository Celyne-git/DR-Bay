...
/datum/category_item/player_setup_item/skills/content()
	...
	for(var/V in SKILLS)
		...
		for(var/datum/skill/S in SKILLS[V])
			...
			dat += "<th><a href='?src=\ref[src];setskill=\ref[S];newvalue=[SKILL_NONE]'><font color=[(level == SKILL_NONE) ? "red" : ""]>\[Low\]</font></a></th>"
			// secondary skills don't have an amateur level
			if(S.secondary)
				dat += "<th></th>"
			else
				dat += "<th><a href='?src=\ref[src];setskill=\ref[S];newvalue=[SKILL_BASIC]'><font color=[(level == SKILL_BASIC) ? "red" : ""]>\[Medium\]</font></a></th>"
			dat += "<th><a href='?src=\ref[src];setskill=\ref[S];newvalue=[SKILL_ADEPT]'><font color=[(level == SKILL_ADEPT) ? "red" : ""]>\[High\]</font></a></th>"
			dat += "<th><a href='?src=\ref[src];setskill=\ref[S];newvalue=[SKILL_EXPERT]'><font color=[(level == SKILL_EXPERT) ? "red" : ""]>\[Extreme\]</font></a></th>"
			dat += "</tr>"
	dat += "</table>"
	. = dat.Join()
